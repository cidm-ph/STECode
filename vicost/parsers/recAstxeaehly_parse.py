...
def recA_input(file, longread):
    ...
    filt_stx_df["Stxtype"], filt_stx_df["Stxsubtype"], filt_stx_df["A or B"] = (
        filt_stx_df["virgene"].str[0:4],
        filt_stx_df["virgene"].str[0:5],
        filt_stx_df["virgene"].str[5],
    )
    if longread == False:
        div = re_stx_df.loc[re_stx_df["#rname"] == "recA_BA000007_3"][
            "meandepth"
        ].values[0]
        filt_stx_df["AvgReadDepth"] = filt_stx_df.groupby("Stxsubtype")[
            "meandepth"
        ].transform("mean")
        filt_stx_df["Normalised"] = filt_stx_df["AvgReadDepth"] / div
        conditions = [
            (filt_stx_df["Normalised"] > 2.2)
            & (filt_stx_df["Stxtype"].str.contains("Stx1" and "Stx2").any()),
            (filt_stx_df["Normalised"] > 2.2)
            & (filt_stx_df["Stxtype"].str.contains("Stx2").any()),
            (filt_stx_df["Normalised"] > 2.2)
            & (filt_stx_df["Stxtype"].str.contains("Stx1").any()),
            (filt_stx_df["Normalised"] <= 2.2)
            & (filt_stx_df["Normalised"] >= 2)
            & (filt_stx_df["Stxtype"] == "Stx2"),
            (filt_stx_df["Normalised"] <= 2.2)
            & (filt_stx_df["Normalised"] >= 2)
            & (filt_stx_df["Stxtype"] == "Stx1"),
            (filt_stx_df["Normalised"] < 2),
        ]
    ...
    sub_stx_df["NN2"] = pd.Categorical(
        sub_stx_df["NN2"], ordered=True, categories=heirarchy
    )
